...
check_6() {
  ...
  if [ -z "$protection" ]; then
    print_fail 6 "No branch protection on ${DEFAULT_BRANCH}. Run Phase 4 to configure."
    return
  fi
  local run_id
  ...
  if [ -n "$missing_checks" ]; then
    print_fail 6 "CI jobs missing from required checks:${missing_checks}. Re-run Phase 4."
    return
  fi
  ...
  if [ "$enforce_admins" != "true" ]; then
    print_fail 6 "enforce_admins is OFF — agents/admins can bypass all branch rules. Re-run Phase 4."
    return
  fi
  ...
  if [ -z "$approval_count" ] || [ "$approval_count" -lt 1 ] 2>/dev/null; then
    print_fail 6 "PR approval count is ${approval_count:-0} — must be at least 1. Re-run Phase 4."
    return
  fi
...
check_7() {
  local merged_url
  merged_url=$(gh pr list --state merged --limit 1 --json url -q '.[0].url' 2>/dev/null) || true
  if [ -n "$merged_url" ]; then
    print_pass 7 "Merged PR found: ${merged_url}"
    return
  fi
  local open_url
  open_url=$(gh pr list --state open --limit 1 --json url -q '.[0].url' 2>/dev/null) || true
  if [ -z "$open_url" ]; then
    print_pending 7 "No PR found. Create one to verify CI gates."
    return
  fi
  local checks_output exit_code
  checks_output=$(gh pr checks "$open_url" 2>&1)
  exit_code=$?
